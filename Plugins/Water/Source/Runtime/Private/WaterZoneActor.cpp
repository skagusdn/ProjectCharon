	...
	, RenderTargetResolution(512, 512)
{
	...
	CharonZoneExtentZ = 16384.; //
	//LocalTessellationExtent = FVector(51200., 51200., 10000.);
	LocalTessellationExtent = FVector(51200., 51200., 16384.);
	...
	// Setup bounds component
	{
		...
		//Charon 수정
		BoundsComponent->SetBoxExtent(FVector(ZoneExtent / 2., CharonZoneExtentZ / 2.));
...
FBox AWaterZone::GetZoneBounds() const
{
	...
	const double WaterZoneZ = GetActorLocation().Z;
...
FVector AWaterZone::GetZoneExtent3D() const
{
	return FVector(ZoneExtent.X, ZoneExtent.Y, CharonZoneExtentZ) * FVector(GetActorScale());
}
...
void AWaterZone::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	// Charon 수정
	if (PropertyName == GET_MEMBER_NAME_CHECKED(AWaterZone, CharonZoneExtentZ))
	{
		OnExtentChanged();
	}
	//
...
void AWaterZone::OnExtentChanged()
{
	...
	BoundsComponent->SetBoxExtent(FVector(GetZoneExtent3D() / 2.));
#endif // WITH_EDITOR
...
bool AWaterZone::UpdateWaterInfoTexture()
{
	if (UWorld* World = GetWorld(); World && FApp::CanEverRender())
	{
		...
		//WaterHeightExtents = FVector2f(WaterZMin, WaterZMax);
		 ...
		 WaterHeightExtents = FVector2f(ZLocation - CharonZoneExtentZ/2., ZLocation + CharonZoneExtentZ/2.);
		//수정 끝!
...
FVector AWaterZone::GetDynamicWaterInfoExtent() const
{
	...
	else
	{
		...
		return FVector(GetZoneExtent3D());
	}
}
