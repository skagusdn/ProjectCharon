...
void UCharonCharacterMovementComponent::PhysSwimming(float deltaTime, int32 Iterations)
{
	...
	if( !HasAnimRootMotion() && !CurrentRootMotion.HasOverrideVelocity() )
	{
		...
		// 1) 부력 + 중력 가속도
		const FVector BuoyancyAccelVec = (GetGravityZ() * (1.f - NetBuoyancy)) * -GetGravityDirection();
		float TotalAccelZ = GetGravitySpaceZ(BuoyancyAccelVec);

		// 2) 댐핑 가속도 (현재 속도에 반대로 작용)
		{
			const float VelZ = GetGravitySpaceZ(Velocity);
			const float FirstOrderDrag = BuoyancyDamp * VelZ;
			const float SecondOrderDrag = FMath::Sign(VelZ) * BuoyancyDamp2 * VelZ * VelZ;
			//TotalAccelZ += -FMath::Max(FirstOrderDrag + SecondOrderDrag, 0.f);
			TotalAccelZ += -(FirstOrderDrag + SecondOrderDrag);
		}

		// 3) 경사 추종 가속도 - 경사를 따라가는 Z 보정 속도. 경사와 수평 속도에 비례. 
		if (GetSwimBuoyancyComponent())
		{
			HorizontalVelocity = {Velocity.X, Velocity.Y, 0.f};
			...
			if (HorizontalSpeed > SlopeFollowMinHorizontalSpeed)
			{
				...
				SlopeAccelZ = FMath::Clamp(Slope * HorizontalSpeed, -MaxSlopeFollowVerticalSpeed, MaxSlopeFollowVerticalSpeed);
				
				//float SlopeCoefficient = (1.f - (GetSwimBuoyancyComponent() ? ImmersionDepth() : 0.0f)) * 3;
				float LessClampedDepth = GetSwimBuoyancyComponent() ? ImmersionDepth() : 0.0f;
				LessClampedDepth = FMath::Max(LessClampedDepth, 0.05);
				float SlopeCoefficient = FMath::Clamp(FMath::Log2(LessClampedDepth) * 2, -4.f, 4.f);
				
				SlopeAccelZ += SlopeCoefficient * HorizontalSpeed * DepthCorrectionGain ;
				
				TotalAccelZ += SlopeAccelZ;
			}
		}
		
		// 4) 합산된 가속도를 한 번만, "더해서" 적용 -> 기존 Z속도(낙하속도 등)는 절대 안 지워짐
		SetGravitySpaceZ(Velocity, GetGravitySpaceZ(Velocity) + TotalAccelZ * deltaTime);
		
		TotalTestVelocity = Velocity;//
	}
