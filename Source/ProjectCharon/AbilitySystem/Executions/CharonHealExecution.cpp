...
struct FHealStatics
{
	...
	FHealStatics()
	{
		BaseHealDef = FGameplayEffectAttributeCaptureDefinition(UCombatAttributeSet::GetHealingAmountAttribute() , EGameplayEffectAttributeCaptureSource::Source, true);
	}
};
...
UCharonHealExecution::UCharonHealExecution()
{
	RelevantAttributesToCapture.Add(HealStatics().BaseHealDef);
}

void UCharonHealExecution::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
#if WITH_SERVER_CODE
